...
<script>
import { reqGetPayUrl, reqGetIfPay } from "../../api/trade";
console.log(reqGetIfPay);
...
export default {
  ...
  methods: {
    async open() {
      ...
      const payUrl = await reqGetPayUrl(orderId);

      QRCode.toDataURL(payUrl.codeUrl)
        .then((url) => {
          this.$confirm("", {
